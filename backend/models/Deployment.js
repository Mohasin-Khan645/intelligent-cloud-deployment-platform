...
class Deployment {
  static async findAll({ projectId, environment } = {}) {
    try {
      ...
      let list = db.memoryStore.deployments.map(d => {
        const project = db.memoryStore.projects.find(p => p.id === d.project_id);
        return {
          ...d,
      ...
      if (projectId) {
        list = list.filter(d => d.project_id === parseInt(projectId, 10));
      }
      if (environment) {
  ...
  static async findById(id) {
    ...
    try {
      ...
      const d = db.memoryStore.deployments.find(item => item.id === deploymentId);
      if (!d) return null;
      const project = db.memoryStore.projects.find(p => p.id === d.project_id);
      return {
        ...d,
        project_name: project ? project.name : 'Unknown Project',
        github_repository: project ? project.github_repository : 'github.com/repo',
        github_branch: project ? project.github_branch : 'main'
      };
    }
  }

  static async create({ project_id, version, commit_sha, environment = 'Production', status = 'QUEUED', error_message = null }) {
    const pId = parseInt(project_id, 10);
    const sha = commit_sha || Math.random().toString(16).substring(2, 9);
    const ver = version || `v1.0.${Math.floor(Math.random() * 90 + 10)}`;

    try {
      const result = await db.query(
        `INSERT INTO deployments (project_id, version, commit_sha, environment, status, started_at, error_message)
         VALUES ($1, $2, $3, $4, $5, CURRENT_TIMESTAMP, $6)
         RETURNING *`,
        [pId, ver, sha, environment, status, error_message]
      );
      const newDep = result.rows[0];

      // Update environment current version if success
      if (status === 'SUCCESS') {
        await db.query(
          `UPDATE environments 
           SET current_version = $1, status = 'Healthy' 
      ...
      const newId = db.memoryStore.deployments.length
        ? Math.max(...db.memoryStore.deployments.map(d => d.id)) + 1
        : 1;
      ...
      if (status === 'SUCCESS') {
        const env = db.memoryStore.environments.find(e => e.project_id === pId && e.name === environment);
        if (env) {
  ...
  static async updateStatus(id, status, errorMessage = null, completedAt = null) {
    ...
    try {
      const result = await db.query(
        `UPDATE deployments 
         ...
         WHERE id = $4 
         RETURNING *`,
        [status, errorMessage, completedAt, depId]
      );
      return result.rows[0] || null;
    } catch (err) {
      const dep = db.memoryStore.deployments.find(d => d.id === depId);
      if (!dep) return null;
      dep.status = status;
  ...
  static getLogs(deployment) {
    ...
    const startTime = new Date(deployment.started_at || Date.now());
    const formatTime = (offsetSeconds) => {
      const t = new Date(startTime.getTime() + offsetSeconds * 1000);
    ...
    if (deployment.status === 'FAILED') {
      logs.push(`[${formatTime(22)}] [ERROR] ${deployment.error_message || 'Container orchestration error: health check failed to respond within 30s'}`);
      logs.push(`[${formatTime(23)}] [WARN]  Deployment aborted. Active cluster reverted to previous stable state.`);
    } else if (deployment.status === 'ROLLED_BACK') {
      logs.push(`[${formatTime(22)}] [WARN]  Degraded health status detected on port 5000.`);
      logs.push(`[${formatTime(24)}] [INFO]  Automated rollback initiated. Previous version restored.`);
    } else if (deployment.status === 'SUCCESS') {
      logs.push(`[${formatTime(22)}] [INFO]  Running pre-flight HTTP probe at /health... HTTP 200 OK`);
      logs.push(`[${formatTime(24)}] [INFO]  Traffic cutover completed to 100% active tasks.`);
      logs.push(`[${formatTime(25)}] [SUCCESS] Deployment completed successfully in 25s.`);
    } else {
      logs.push(`[${formatTime(21)}] [INFO]  Currently executing step: ${deployment.status}...`);
    }
